...
class UserModel {
    static readUser = () => {
        const mapedUsers = pasteleria_json_1.default.employees.map(employee => ({
    ...
    static deleteUser = (username) => {
        const user = pasteleria_json_1.default.employees.find((n) => n.username.toLowerCase() === username.toLowerCase());
        if (!user) {
            return { error: "user not found" };
        }
        const employees = pasteleria_json_1.default.employees.filter((employee) => employee.username.toLowerCase() !== username.toLowerCase());
    ...
    static login = (objUser) => {
        try {
            ...
            const user = pasteleria_json_1.default.employees.find((u) => u.username.toLowerCase() === username.toLowerCase());
            if (!user)
                return 404;
